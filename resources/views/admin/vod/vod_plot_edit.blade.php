                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <form role="form" method="post" id="myForm">
                                <input type="hidden" value="{{$vod_plot->id}}" id="id" name="id">
                                <div class="form-group">
                                    <label>剧情名称</label>
                                    <input type="text" name="name" placeholder="剧情名称" class="form-control" style="width: 40%;" required aria-required="true" value="{{$vod_plot->name}}">
                                </div>
                                <div class="form-group">
                                    <label>所属分类</label>
                                    <div class="input-group">
                                        <select  class="form-control" style=" width: 400px; padding-bottom: 0px; padding-top: 0px;" tabindex="2" name="pid">
                                            ...
                                            @foreach($cloumns as $item)
                                                <option value="{{$item->id}}" hassubinfo="true" @if($vod_plot->pid == $item->id)selected @endif>{{$item->name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
...
<script>
    function save() {
        ...
        $.ajax({
            ...
            url:"{{route('admin.vod.vod_plot_edit')}}",
            type:'post',
            data:formData,
            timeout:0,
            datatype:'text',
